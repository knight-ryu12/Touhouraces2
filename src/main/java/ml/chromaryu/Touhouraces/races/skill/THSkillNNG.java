...
public class THSkillNNG implements Listener {
    ...
    public static void mazyo_fire(Player pl, final Plugin plugin, final PlayerInteractEvent event) {
        ...
        MetadataValue shooter = new FixedMetadataValue(THRPlugin.plugin, pl.getUniqueId().toString());
        MetadataValue fireeffect = new FixedMetadataValue(THRPlugin.plugin, 30D);
        snowball.setMetadata("fireffect", fireeffect);
        snowball.setMetadata("mazyo-fireball", shooter);
        snowball.setVelocity(velocity);
        snowball.setFireTicks(300);
        MetadataValue usingmagic = new FixedMetadataValue(THRPlugin.plugin, true);
        pl.setMetadata("using-magic", usingmagic);
    ...
    public static void mazyo_thunder(Player pl, final Plugin plugin, final PlayerInteractEvent event) {
        ...
        MetadataValue lightningeffect = new FixedMetadataValue(THRPlugin.plugin, 20D);
        MetadataValue shooter = new FixedMetadataValue(THRPlugin.plugin, pl.getUniqueId().toString());
    ...
    public static void gennzinnsin_luckyattack(Player pl, final Plugin plugin, final EntityDamageByEntityEvent event) {
        if (Math.random() > 0.7 && THRPlugin.plugin.getConfig().getInt("user." + pl.getUniqueId() + ".spilit") >= 5D) {
            THRPlugin.plugin.getConfig().set(THRPlugin.plugin.getConfig().getString("user." + pl.getUniqueId() + ".spilit"), THRPlugin.plugin.getConfig().getInt("user." + pl.getUniqueId() + ".spilit") - 5D);
    ...
    public static void houraizin_reverselife_Entity(Player pl, final Plugin plugin, final EntityDamageByEntityEvent event) {
        ...
        if (event.getDamage() >= pl.getHealth() && reverse > 0.6) {
            THRPlugin.plugin.getConfig().set(THRPlugin.plugin.getConfig().getString("user." + pl.getUniqueId() + ".spilit"), THRPlugin.plugin.getConfig().getInt("user." + pl.getUniqueId() + ".spilit") - 30D);
            pl.setHealth(pl.getMaxHealth());
